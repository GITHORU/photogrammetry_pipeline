...
class PhotogrammetryGUI(QWidget):
    ...
    def export_job_dialog(self):
        ...
        cli_cmd = " ".join(cmd)
        dialog = JobExportDialog(self, job_name="PhotoGeoAlign_MicMac", output="PhotoGeoAlign_MicMac.out", cli_cmd=cli_cmd)
    ...
    def export_geodetic_job_dialog(self):
        ...
        if getattr(sys, 'frozen', False):
            ...
            cmd = [exe_path, script_path] + filtered_args
        cli_cmd = " ".join(cmd)
        dialog = JobExportDialog(self, job_name="PhotoGeoAlign_Geodetic", output="PhotoGeoAlign_Geodetic.out", cli_cmd=cli_cmd)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            vals = dialog.get_values()
    ...
    def export_new_job_dialog(self):
        ...
        if getattr(sys, 'frozen', False):
            ...
            cmd = [exe_path, script_path] + filtered_args
        cli_cmd = " ".join(cmd)
        dialog = JobExportDialog(self, job_name="PhotoGeoAlign_New", output="PhotoGeoAlign_New.out", cli_cmd=cli_cmd)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            vals = dialog.get_values()
