#!/usr/bin/env python3
"""
PhotoGeoAlign - Pipeline de photogrammÃ©trie avec MicMac
Version refactorisÃ©e avec structure modulaire
"""
...
def check_micmac_or_quit():
    try:
        ...
        msg.setWindowTitle("MicMac non dÃ©tectÃ©")
        msg.setTextFormat(Qt.TextFormat.RichText)
        msg.setTextInteractionFlags(Qt.TextInteractionFlag.TextBrowserInteraction)
        msg.setText("Le logiciel MicMac (mm3d) n'est pas installÃ© ou n'est pas accessible dans le PATH systÃ¨me.<br><br>" +
                   "<b>Si vous Ãªtes sous Linux, il est recommandÃ© de lancer PhotoGeoAlign depuis un terminal pour garantir que mm3d soit bien trouvÃ© dans le PATH.</b><br><br>" +
...
main_window = None  # Variable globale pour conserver la fenÃªtre principale
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        # Redimensionne le logo Ã  300px de large (hauteur ajustÃ©e automatiquement)
        target_width = 300
        ...
        parser.add_argument('input_dir', nargs='?', default=None, help="Dossier d'images Ã  traiter")
        parser.add_argument('--mode', default='BigMac', choices=['QuickMac', 'BigMac', 'MicMac'], help='Mode de densification C3DC (dÃ©faut: BigMac)')
        parser.add_argument('--zoomf', type=int, default=1, help='Facteur de zoom (rÃ©solution) pour C3DC (1=max)')
        parser.add_argument('--tapas-model', default='Fraser', help='ModÃ¨le Tapas Ã  utiliser (dÃ©faut: Fraser)')
        parser.add_argument('--tapioca-extra', default='', help='ParamÃ¨tres supplÃ©mentaires pour Tapioca (optionnel)')
        parser.add_argument('--tapas-extra', default='', help='ParamÃ¨tres supplÃ©mentaires pour Tapas (optionnel)')
        parser.add_argument('--c3dc-extra', default='', help='ParamÃ¨tres supplÃ©mentaires pour C3DC (optionnel)')
        parser.add_argument('--saisieappuisinit-pt', default='', help='Chemin du fichier de points d\'appui pour SaisieAppuisInit (optionnel)')
        parser.add_argument('--saisieappuisinit-extra', default='', help='ParamÃ¨tres supplÃ©mentaires pour SaisieAppuisInitQT (optionnel)')
        parser.add_argument('--saisieappuispredic-extra', default='', help='ParamÃ¨tres supplÃ©mentaires pour SaisieAppuisPredicQT (optionnel)')
        parser.add_argument('--skip-saisieappuisinit', action='store_true', help='Ne pas exÃ©cuter SaisieAppuisInit')
        parser.add_argument('--skip-saisieappuispredic', action='store_true', help='Ne pas exÃ©cuter SaisieAppuisPredic')
        parser.add_argument('--skip-tapioca', action='store_true', help='Ne pas exÃ©cuter Tapioca')
        parser.add_argument('--skip-tapas', action='store_true', help='Ne pas exÃ©cuter Tapas')
        parser.add_argument('--skip-c3dc', action='store_true', help='Ne pas exÃ©cuter C3DC')
        
        # Arguments pour les transformations gÃ©odÃ©siques
        parser.add_argument('--geodetic', action='store_true', help='Lancer les transformations gÃ©odÃ©siques')
        parser.add_argument('--geodetic-coord', default='', help='Fichier de coordonnÃ©es de recalage pour les transformations gÃ©odÃ©siques')
        parser.add_argument('--deformation-type', default='tps', choices=['tps'], help='Type de dÃ©formation (dÃ©faut: tps)')
        parser.add_argument('--deformation-params', default='', help='ParamÃ¨tres de dÃ©formation (optionnel)')
        ...
        parser.add_argument('--skip-add-offset', action='store_true', help='Ne pas exÃ©cuter l\'ajout d\'offset')
        parser.add_argument('--skip-itrf-to-enu', action='store_true', help='Ne pas exÃ©cuter la conversion ITRF vers ENU')
        parser.add_argument('--skip-deform', action='store_true', help='Ne pas exÃ©cuter la dÃ©formation')
        parser.add_argument('--skip-orthoimage', action='store_true', help='Ne pas exÃ©cuter la crÃ©ation d\'orthoimage')
        parser.add_argument('--skip-unified-orthoimage', action='store_true', help='Ne pas exÃ©cuter la crÃ©ation d\'orthoimage et MNT unifiÃ©s')

        
        # Arguments pour les dossiers d'entrÃ©e personnalisÃ©s
        parser.add_argument('--add-offset-input-dir', default='', help='Dossier d\'entrÃ©e personnalisÃ© pour l\'ajout d\'offset (optionnel)')
        parser.add_argument('--itrf-to-enu-input-dir', default='', help='Dossier d\'entrÃ©e personnalisÃ© pour ITRF vers ENU (optionnel)')
        parser.add_argument('--deform-input-dir', default='', help='Dossier d\'entrÃ©e personnalisÃ© pour la dÃ©formation (optionnel)')
        parser.add_argument('--orthoimage-input-dir', default='', help='Dossier d\'entrÃ©e personnalisÃ© pour l\'orthoimage (optionnel)')
        parser.add_argument('--unified-orthoimage-input-dir', default='', help='Dossier d\'entrÃ©e personnalisÃ© pour l\'orthoimage unifiÃ©e (optionnel)')

        
        # Arguments pour les dossiers de sortie personnalisÃ©s
        parser.add_argument('--add-offset-output-dir', default='', help='Dossier de sortie personnalisÃ© pour l\'ajout d\'offset (optionnel)')
        parser.add_argument('--itrf-to-enu-output-dir', default='', help='Dossier de sortie personnalisÃ© pour ITRF vers ENU (optionnel)')
        parser.add_argument('--deform-output-dir', default='', help='Dossier de sortie personnalisÃ© pour la dÃ©formation (optionnel)')
        parser.add_argument('--orthoimage-output-dir', default='', help='Dossier de sortie personnalisÃ© pour l\'orthoimage (optionnel)')
        parser.add_argument('--unified-orthoimage-output-dir', default='', help='Dossier de sortie personnalisÃ© pour l\'orthoimage unifiÃ©e (optionnel)')

        
        # Arguments pour les paramÃ¨tres d'orthoimage
        parser.add_argument('--orthoimage-resolution', type=float, default=0.1, help='RÃ©solution de l\'orthoimage en mÃ¨tres (dÃ©faut: 0.1)')
        ...
        parser.add_argument('--color-fusion-median', action='store_true', help='Utiliser la mÃ©thode de mÃ©diane pour la fusion des couleurs')

        parser.add_argument('--max-workers', type=int, default=4, help='Nombre maximum de processus parallÃ¨les (dÃ©faut: 4)')
    
        args = parser.parse_args()
        if args.geodetic:
            # Mode transformations gÃ©odÃ©siques
            if not args.input_dir or not os.path.isdir(args.input_dir):
                print("Erreur : veuillez spÃ©cifier un dossier de nuages valide.")
                sys.exit(1)
            if not args.geodetic_coord or not os.path.exists(args.geodetic_coord):
                print("Erreur : veuillez spÃ©cifier un fichier de coordonnÃ©es valide.")
                sys.exit(1)
            
            log_path = os.path.join(args.input_dir, 'geodetic_transforms.log')
            logger = setup_logger(log_path)
            print(f"DÃ©but des transformations gÃ©odÃ©siques pour le dossier : {args.input_dir}")
            ...
            try:
                ...
                # Dossiers d'entrÃ©e personnalisÃ©s
                ...
                # Dossiers de sortie personnalisÃ©s
                ...
                print("Transformations gÃ©odÃ©siques terminÃ©es avec succÃ¨s !")
            except Exception as e:
                print(f"Erreur lors des transformations gÃ©odÃ©siques : {e}")
                sys.exit(1)
        elif args.no_gui:
            # Mode pipeline photogrammÃ©trique
            check_micmac_or_quit()
            if not args.input_dir or not os.path.isdir(args.input_dir):
                print("Erreur : veuillez spÃ©cifier un dossier d'images valide.")
                sys.exit(1)
            log_path = os.path.join(args.input_dir, 'photogrammetry_pipeline.log')
            logger = setup_logger(log_path)
            print(f"DÃ©but du pipeline photogrammÃ©trique pour le dossier : {args.input_dir}")
            try:
                ...
                print("Pipeline terminÃ© avec succÃ¨s !")
            except Exception as e:
                print(f"Erreur lors de l'exÃ©cution du pipeline : {e}")
                sys.exit(1)
        else:
            check_micmac_or_quit()
            app = QApplication(sys.argv)
